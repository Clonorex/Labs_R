...
X.matrixlog <- model.matrix(y ~ x1 * FO, data = lg)
head(X.matrixlog)

data.fitlog <- cbind(y = lg$y, 
...
X.matrixlog <- model.matrix(y ~ x2 * FO, data = lg)
data.fitlog <- cbind(y = lg$y, 
                     data.frame(X.matrixlog)[, -1])
head(data.fitlog)
tail(data.fitlog)
...
save(list = c('data.fit.X1.fo', 'data.fit.X2.fo', 'DF', 'reg.df',
              'models.list', 'data.fitlog.X1.fo', 'data.fitlog.X2.fo', 'DF', 'lg',
              'modelslog.list'), 
